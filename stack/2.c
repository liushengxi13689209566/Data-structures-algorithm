...
{
    char operation = Symbol[*SymbolTop--]; //出符号，必进行计算
    int value1=number[*numberTop--];  //出两个数进行计算
    int value2=number[*numberTop--];
    int temp;
    switch(operation)
    {
        case '+':temp=value2+value1;  break; //顺序一定是values2 '/' 或者 '-' values1
        case '-':temp=value2 -value1; break;
        case '*':temp =value2*value1; break;
        case '/':temp =value2/value1; break;
    }
//    *numberTop++;
  //  number[*numberTop] = temp ;  //把结果再压进数值栈中
  return temp ;
}
    ...
    while(str[i]){  //先遍历该字符串
        ...
        if(str[i] <= '9' && str[i] >= '0'){ //是数字
            ...
            //从符号栈中出一个符号，从数值栈中出两个数字，计算后压入数值栈
                    temp=cal(number,&numberTop,Symbol,&SymbolTop);
